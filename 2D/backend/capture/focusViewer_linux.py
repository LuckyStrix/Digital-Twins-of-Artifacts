#!/usr/bin/env python3
"""
focusViewer_linux.py
====================
Linux port of focusViewer.py — the focus-check helper for the capture rig. It
turns on all four directional lights and opens a small window showing a live-view
preview frame from the camera so the operator can set focus before a real
capture.
...
Differences from the Windows version:
  - The serial port defaults to /dev/ttyACM0 (overridable with
    PAPYRUS_SERIAL_PORT) instead of COM3.
  - gphoto2 is called directly instead of through msys2_shell.cmd.

Launched by run_linux.py's "Open Focus Viewer" button. Requires the same
hardware as the capture stage (camera over USB + Arduino on a serial port) plus
gphoto2 on PATH, and Pillow for displaying the preview.
"""
...
# ── Hardware config — keep in sync with arduinoIntegration_linux.py ──────────
# Serial port: override with PAPYRUS_SERIAL_PORT, else default to /dev/ttyACM0
# and fall back to the first detected ttyACM*/ttyUSB* if that is absent.
...
def _resolve_serial_port() -> str:
    ...
    return candidates[0] if candidates else DEFAULT_SERIAL_PORT
...
class FocusViewer:
    ...
    def _capture_preview(self):
        ...
        # On Linux gphoto2 is native, so call it directly (no msys2 shim).
        subprocess.run(
