// Stepper motor controller for T-Capture turntable, plus LED lighting control.
// Motor: 8-byte serial commands from the PC: [4-digit num_captures][4-digit steps]
//   Steps the motor by <steps> pulses, then sends 'e' to signal completion.
// LEDs: single-byte commands 'N' (on) / 'F' (off) drive the transistor on
//   LED_PIN, acknowledged with 'e' the same way as motor moves.
...
void setup() {
  ...
  digitalWrite(LED_PIN,       LOW);   // LEDs off at startup
}
...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (c == 'N' || c == 'F') {
      Serial.read();
      digitalWrite(LED_PIN, c == 'N' ? HIGH : LOW);
