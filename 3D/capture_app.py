...
class CaptureApp:
    def __init__(self, root):
        self.root = root
        self.root.title("T-Capture Multi-Camera Scanner")
    ...
    def _run_capture(self):
        ...
        for side in (1, 2):
            ...
            for i in range(caps_int):
                ...
                # Capture each camera sequentially; check results and retry misses.
                results = self._capture_all(cam_ports)
                for name, port in zip(cam_names, cam_ports):
                    ok, msg = results[port]
                    if not ok:
    ...
    def _capture_all(self, cam_ports):
        """Trigger each camera one at a time and wait for each to finish.

        Concurrent gphoto2 capture on a shared USB bus fired the shutters but
        the frames never made it onto the cards, so capture is fully sequential.
        Each camera's result is checked and any that errors is retried once.
